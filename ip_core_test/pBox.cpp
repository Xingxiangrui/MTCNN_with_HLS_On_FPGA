...
void pBoxShowE(const struct pBox *pbox,int channel, int row){
	...
	cout << "the data is :" << endl;
	volatile float *p = pbox->pdata + channel*pbox->width*pbox->height;
	//pbox->channel
	cout << "the " << channel <<"th channel data is :"<< endl;
	//pbox->height
...
void weightShow(const struct Weight *weight){
	...
	volatile float *p = weight->pdata;
	for (int channel = 0; channel < weight->out_ChannelNum; channel++){
		cout << "the " << channel <<"th channel data is :"<< endl;
		for (int i = 0; i < weight->in_ChannelNum; i++){
