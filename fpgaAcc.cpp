...
int max_out_channelNum = 0;
int Max_input_image_pixels = 0;
...
#pragma HLS INTERFACE m_axi depth=DRAM_DEPTH port=output_ptr offset=slave bundle=memorybus
	...
	layer_setup:{
		...
		BiasCache::setLayerConfig(OutChanNum);
	};
	
	//load weights from DRAM to WBRAM
	WeightsCache::load_WBRAM_from_DRAM(weight_ptr);

	//load bias from DRAM to BBRAM
	BiasCache::load_BBRAM_from_DRAM(weight_bias);
	...
	row_loop:for(cur_row_out=0;cur_row_out<MemoryController::out_height;cur_row_out++){//out_row,out_height
	  ...
	  col_loop:for(cur_col_out=0;cur_col_out<MemoryController::out_width;cur_col_out++){//out_col,out_width
		...
		ch_out_loop:for(cur_channel_out=0; cur_channel_out<MemoryController::out_channelNum; cur_channel_out++){
			MemoryController:: writeBackOutputChannel_test(output_ptr,cur_channel_out, \
...
//write from OBRAM to DRAM add bias and ReLU
void MemoryController::writeBackOutputChannel_test(volatile float * output_ptr,int cur_co,
	                                               float data) {
#pragma HLS inline
  int channel_offset=cur_co*out_channelPixels;
#pragma HLS RESOURCE variable = channel_offset core = MulnS latency = 2 
  data += BiasCache::BBRAM[cur_co];
#pragma HLS RESOURCE variable = BBRAM core = RAM_S2P_BRAM	  
  float relu_data;
  relu_data = data>0?data:0;
  output_ptr[channel_offset+peixl_out_DRAM_offset] = (volatile float) relu_data;
};
...
//------------------------BiasCache---------------------------
void BiasCache::setLayerConfig(int OutChanNum){
#pragma HLS inline
	BiasCache::outChannelNum=OutChanNum;
};

void BiasCache::load_BBRAM_from_DRAM(volatile float * weight_bias){
#pragma HLS inline
	for(int cur_co=0;cur_co<outChannelNum;cur_co++){
		BBRAM[cur_co] = weight_bias[cur_co];
	}
};
